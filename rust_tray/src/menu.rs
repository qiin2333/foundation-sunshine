...
// ============================================================================
// Backward compatibility functions using item IDs from menu_items::ids
// ============================================================================

use crate::actions::MenuAction;

/// Set check state by MenuAction (for C API compatibility)
pub fn set_check_state(action: MenuAction, checked: bool) {
    if let Some(item_id) = action_to_item_id(action) {
        set_check_state_by_id(item_id, checked);
    }
}

/// Identify MenuAction from menu event (for lib.rs compatibility)
pub fn identify_action(event: &MenuEvent) -> Option<MenuAction> {
    identify_item_id(event).and_then(|id| item_id_to_action(&id))
}

/// Map MenuAction to item_id
fn action_to_item_id(action: MenuAction) -> Option<&'static str> {
    use menu_items::ids::*;
    match action {
        MenuAction::OpenUI => Some(OPEN_SUNSHINE),
        MenuAction::VddCreate => Some(VDD_CREATE),
        MenuAction::VddClose => Some(VDD_CLOSE),
        MenuAction::VddPersistent => Some(VDD_PERSISTENT),
        MenuAction::ImportConfig => Some(IMPORT_CONFIG),
        MenuAction::ExportConfig => Some(EXPORT_CONFIG),
        MenuAction::ResetConfig => Some(RESET_CONFIG),
        MenuAction::CloseApp => Some(CLOSE_APP),
        MenuAction::ResetDisplayDeviceConfig => Some(RESET_DISPLAY),
        MenuAction::LanguageChinese => Some(LANG_CHINESE),
        MenuAction::LanguageEnglish => Some(LANG_ENGLISH),
        MenuAction::LanguageJapanese => Some(LANG_JAPANESE),
        MenuAction::StarProject => Some(STAR_PROJECT),
        MenuAction::VisitProjectSunshine => Some(VISIT_SUNSHINE),
        MenuAction::VisitProjectMoonlight => Some(VISIT_MOONLIGHT),
        MenuAction::Restart => Some(RESTART),
        MenuAction::Quit => Some(QUIT),
        // NotificationClicked is not a menu item
        MenuAction::NotificationClicked => None,
    }
}

/// Map item_id to MenuAction
fn item_id_to_action(item_id: &str) -> Option<MenuAction> {
    use menu_items::ids::*;
    match item_id {
