...
namespace system_tray_i18n {
  ...
  const std::map<std::string, std::string> DEFAULT_STRINGS = {
    ...
    { KEY_VDD_HEADLESS_CREATE_CONFIRM_MSG, "1. This is an internal beta feature. It works differently from \"Keep Enabled\".\n\n2. It only runs when Sunshine starts or when a stream ends; on headless hosts it can create the base display after stream end to avoid app issues.\n\n3. If you later connect a physical display, the screen might show only on the base display (physical screen black). Use Ctrl+Alt+Win+B to restore, or start then end a stream to release the base display.\n\nEnable this feature?" },
  ...
  const std::map<std::string, std::string> CHINESE_STRINGS = {
    ...
    { KEY_VDD_HEADLESS_CREATE_CONFIRM_MSG, "1. 此为内测功能，请知悉。与「保持启用」原理不同。\n\n2. 仅在 Sunshine 启动或串流结束时检测；无头主机退出串流后若无显示器，将自动创建基地显示器，避免部分应用异常。\n\n3. 若之后接回物理显示器，可能被设为仅基地显示器可见而黑屏，可按 Ctrl+Alt+Win+B 恢复显示，或重新串流后再结束以释放基地显示器。\n\n确定启用？" },
  ...
  const std::map<std::string, std::string> JAPANESE_STRINGS = {
    ...
    { KEY_VDD_HEADLESS_CREATE_CONFIRM_MSG, "1. 内側テスト用のβ機能です。「常駐」とは仕様が異なります。\n\n2. Sunshine 起動時またはストリーム終了時のみ検出。ヘッドレスでストリーム終了後にディスプレイが無い場合、基地ディスプレイを自動作成し、一部アプリの不具合を防ぎます。\n\n3. 後に物理ディスプレイを接続すると、表示が基地ディスプレイのみ（物理が黒画面）になる場合があります。Ctrl+Alt+Win+B で復元するか、ストリームを開始して終了すると基地ディスプレイが解放されます。\n\n有効にしますか？" },
